var CrearGema = Vue.component('CrearGema', {
    ...
    <div>
        <button v-on:click="validarFormulario(), crear_gema(nombre, poder, portador, ubicacion)" class="btn-form">Agregar Gema</button>
    </div>
    ...
    methods: { //Inician los Métodos
        ...
        crear_gema: function (nombre, poder, portador, ubicacion) {
            if (!(Array.isArray(this.errores) && this.errores.length)) {
                let self = this;
                axios.post('https://api-avengers.firebaseio.com/gemas.json', {
                    nombre: nombre,
                    poder: poder,
                    portador: portador,
                    ubicacion: ubicacion
